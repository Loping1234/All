...
export function RateLimitIndicator() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setStatus(newStatus);
      const maxRequestsPerMinute = Math.max(1, newStatus.requestCount + newStatus.requestsRemaining);
      
      // Show indicator if queue is active or approaching limit
      const shouldShow = 
        newStatus.queueLength > 0 || 
        newStatus.isProcessing || 
        newStatus.requestsRemaining < maxRequestsPerMinute;
  ...
  const maxRequestsPerMinute = Math.max(1, status.requestCount + status.requestsRemaining);
  const lowRequestsThreshold = Math.max(1, Math.floor(maxRequestsPerMinute * 0.4));
  const warningRequestsThreshold = Math.max(lowRequestsThreshold + 1, Math.ceil(maxRequestsPerMinute * 0.6));
  const progressWidth = (status.requestsRemaining / maxRequestsPerMinute) * 100;

  const getStatusColor = () => {
    if (status.requestsRemaining <= lowRequestsThreshold) return 'from-red-500 to-orange-500';
    if (status.requestsRemaining <= warningRequestsThreshold) return 'from-yellow-500 to-orange-500';
  ...
  const getStatusIcon = () => {
    ...
    if (status.requestsRemaining <= lowRequestsThreshold) {
        ...
        <div className="glass-panel-strong rounded-2xl px-4 py-3 border border-white/20 shadow-xl">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              {status.requestsRemaining < maxRequestsPerMinute && (
          ...
          {status.requestsRemaining <= lowRequestsThreshold && (
            <motion.div
