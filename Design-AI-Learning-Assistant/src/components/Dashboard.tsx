...
export function Dashboard() {
  ...
  const startAttentionSession = () => {
    window.location.href = "http://localhost:5000/";
  };

  // Deterministic "next best step" derived from what the user already has
  const nextAction = uploadedDocuments.length === 0
    ? {
        icon: Upload,
        title: "Upload your first study document",
        description: "Everything starts with your material — quizzes, flashcards, and the AI tutor all build on what you upload.",
        cta: "Upload Now",
    ...
    : generatedQuiz
      ? {
          icon: Play,
          title: "Your quiz is ready to take",
          description: `${generatedQuiz.questions.length} questions are waiting — take it while the material is still fresh.`,
          cta: "Start Quiz",
          onClick: () => setActiveSection("quiz"),
        }
      : {
          icon: Activity,
          title: "Start a live study session",
          description: "Put your focus to the test — real-time attention tracking keeps you honest while you study.",
          cta: "Start Session",
          onClick: startAttentionSession,
        };

  const sectionHeader = (icon: React.ElementType, label: string, hint: string) => {
    const Icon = icon;
    return (
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          className="glass-panel-strong rounded-3xl p-6 md:p-8 mb-12 relative overflow-hidden border border-purple-500/20"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.1 }}
        >
          <div className="absolute inset-0 bg-gradient-to-r from-purple-600/15 via-transparent to-blue-600/15 pointer-events-none" />
          <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center flex-shrink-0 shadow-lg shadow-purple-500/30">
              <nextAction.icon className="w-7 h-7 text-white" />
            </div>
            <div className="flex-1">
              <p className="text-sm uppercase tracking-widest text-purple-300 mb-1">Recommended next step</p>
              <h3 className="text-2xl text-white mb-1">{nextAction.title}</h3>
              <p className="text-white/60">{nextAction.description}</p>
            </div>
            <GlassButton onClick={nextAction.onClick} size="lg">
              {nextAction.cta}
        ...
        {/* Prepare: get material into the system */}
        <div className="mb-12">
          {sectionHeader(FolderOpen, "Prepare", "Bring in your study material")}
        ...
        {/* Practice: turn material into mastery */}
        <div className="mb-12">
          {sectionHeader(Target, "Practice", "Turn material into mastery")}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <DashboardCard
              icon={Brain}
              title="Quizzes"
        ...
        {/* Execute: study for real and track progress */}
        <div className="mb-12">
          {sectionHeader(Rocket, "Execute", "Study for real, with focus tracking")}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <DashboardCard
              icon={Activity}
              title="Attention Monitor"
              description="Start a monitored study session to track your focus levels"
              gradient="from-emerald-500 to-teal-500"
              onClick={startAttentionSession}
              delay={0.4}
            >
              <GlassButton variant="ghost" size="sm" onClick={startAttentionSession}>
                Start Session
              </GlassButton>
            </DashboardCard>
